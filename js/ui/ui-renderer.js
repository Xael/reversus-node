...
/**
 * Renders the player's hand into the hand overlay modal.
 * @param {object} player - The player object whose hand is being rendered.
 */
export const renderHandOverlay = (player) => {
    const { gameState } = getState();
    if (!dom.cardFan) return;
    
    dom.cardFan.innerHTML = player.hand.map(card => {
        // We reuse the existing renderCard function
        let cardHTML = renderCard(card, 'overlay-hand', player.id);
        
        // If this card is selected, inject the action buttons
        if (gameState.selectedCard?.id === card.id) {
            const buttonsHTML = `
                <div class="card-actions">
                    <button class="play-action-btn" data-action="play">${t('game.play')}</button>
                    <button class="back-action-btn" data-action="back">${t('common.back')}</button>
                </div>
            `;
            // Inject inside the card's div
            return cardHTML.slice(0, -6) + buttonsHTML + '</div>';
        }
...
export const updateActionButtons = () => {
    ...
    if (!gameState || !dom.showHandButton) return;
    ...
    dom.showHandButton.disabled = !isMyTurn;
    dom.endTurnButton.disabled = !isMyTurn;

    // Add glowing effect
    dom.showHandButton.classList.toggle('glowing', isMyTurn);
    dom.endTurnButton.classList.toggle('glowing', isMyTurn);
};
